...
space = [
    Integer(21, 22, name='k'),
    Integer(3, 4, name='d'),
    Real(1e-5, 1e-1, prior='log-uniform', name='learning_rate'),
    Integer(1000, 1001, name='n_epochs'),
...
def train_once(params, make_plot=False, plot_dir=None, wandb_project='flywriteSBM6_poisson'):
    run = wandb.init(
    ...
    if make_plot:
        if plot_dir is None:
            ...
            plot_dir = Path('elbo_plots') / f'best_run_{run_name}'
        else:
...
def objective(**params):
    lowest_elbo, _, _ = train_once(params, make_plot=False)
    return lowest_elbo
...
if __name__ == '__main__':
    ...
    best_params = {
        ...
        "n_epochs": int(res_gp.x[3]),
        "optimizer": str(res_gp.x[4]),
    }

    best_plot_dir = Path('elbo_plots') / 'best_hyperparameter_run'
    ...
    best_loss, best_epoch, _ = train_once(
        ...
        wandb_project='flywriteSBM6_poisson_best_run',
    )
