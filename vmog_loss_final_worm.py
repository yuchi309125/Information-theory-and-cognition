...
def pick_device():
    if torch.cuda.is_available():
        return "cuda"
    # mps only exists on macOS builds; guard it hard
...
def run_pca_gmm(batch, index, device="cuda"):
    """
    PCA-free version of the previously commented run_pca_gmm.
    - Loads the sparse connectivity matrix
    - Runs BatchedEMGaussianMixture directly on the raw (dense) features
    - Saves artifacts with the same filenames as before for compatibility:
        * U_orth_tuned_{iteration}.pt  (stores raw dense features here)
        * vmog_cluster_centers_tuned_{it}.pt
        * vmog_labels_tuned_{it}.pt
        * vmog_cluster_assignment_dict_tuned_{it}.npy
    """
    import gc
    ...
    print(f"[run_pca_gmm-noPCA] Loaded sparse matrix with shape {adj_matrix.shape} and {adj_matrix.nnz} non-zero entries.")
    ...
    if labels is not None and cluster_centers is not None:
        ...
        for i in range(len(labels)):
            root_id = mapping[i]
            cluster_assignment_dict[root_id] = int(labels[i].item())

        Path("vmog_runs_worm").mkdir(parents=True, exist_ok=True)
        torch.save(X.cpu(), f"vmog_runs_worm/U_orth_tuned_{iteration}.pt")  # 这里存原始 dense 特征
        print("Saved raw feature matrix (as U_orth) to disk.")

        torch.save(cluster_centers.cpu(), f"vmog_runs_worm/vmog_cluster_centers_tuned_{iteration}.pt")
        torch.save(labels.cpu(), f"vmog_runs_worm/vmog_labels_tuned_{iteration}.pt")
        np.save(f"vmog_runs_worm/vmog_cluster_assignment_dict_tuned_{iteration}.npy",
                cluster_assignment_dict, allow_pickle=True)

        del labels, cluster_centers, mapping, cluster_assignment_dict

    del X, X_np, gmm_model
    gc.collect()
